...
function tojiariJson(obj) {
        jia = {...obj.holiday};
        const result = {};
        for (const key in jia) {
            if (jia.hasOwnProperty(key)) {
                const newKey = jia[key].date; // 也可以修改Key
                const newValue = jia[key].name;
...
function getHoliday(date) {
  ...
  $.ajax({
    type: "GET",
    url: "https://timor.tech/api/holiday/year/" + s_year,
    dataType: "json",
    jsonpCallback: "call",
    success: function (data) {
      holidays = tojiariJson(data);
    },
    error: function () {
      holidayInfoEl.textContent = '获取节假日数据失败';
    }
  });
}
...
function getAllDivsHtmlByClass(datastr, className) {
  const tempDiv = document.createElement('div'); 
  tempDiv.innerHTML  = datastr;
  
  const targetDivs = tempDiv.querySelectorAll(`.${className}`); 
  return Array.from(targetDivs).map(div  => div.innerHTML); 
}

function getJieQi(date) {
  const s_year = date.getFullYear();
  $.ajax({
      url: 'https://jieqi.bmcx.com/' + s_year + '__jieqi/',
      async: false,
      success: function(data) {
          var airdata = getAllDivsHtmlByClass(data,'div.jieqi_xia');
          console.log(airdata);
      },
      error: function(data) {
...
function getJieqi(date) {
  ...
  (new Ajax(null
      ...
      ,10 * 1000))
  .send("/" + s_year + "__jieqi", null, "GET", true);
  return airdata;
}
...
function getCalendar(date) {
  ...
  const s_month = date.getMonth() + 1;
  var y = s_year;
  var m = s_month < 10 ? '0' + s_month : s_month.toString();
  var ym = y + '-' + m;
  $.ajax({
      url: 'https://weather.cma.cn/',
      async: false,
      success: function(data) {
          var airdata = getAllDivsHtmlByClass(data,'wea_weather clearfix');
          console.log(airdata);
          if(airdata.length > 0) {
              $('#calendar_grid').html(airdata[0]);
